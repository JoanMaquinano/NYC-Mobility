...
def list_files(path):
    ...
    try:
        return sorted((f.name, f.modificationTime) for f in dbutils.fs.ls(path) if not f.name.endswith("/"))
    except Exception:
        return []
...
for name, mtime in TAXI_FILES:
    for m in re.findall(r"(\d{4}-\d{2})", name):
        landed[m] = mtime
if not landed:
    raise FileNotFoundError(f"No green_tripdata_YYYY-MM.parquet files in {TAXI_DIR} — nothing to check.")
...
if BATCH_MONTH not in weather_months:
    print(f"   note: no weather file for {BATCH_MONTH} — " f"weather will be held back, taxi proceeds")
...
if _zone_files:
    ...
    STATIC_BATCH = "static-" + datetime.fromtimestamp(_zone_mtime / 1000).strftime("%Y-%m-%d")
else:
...
def record(
    table, category, name, failed, total, threshold=STRICT, warn=0.0, min_rows=0, blocking=False, is_empty_check=False
):
    ...
    RESULTS.append(
        dict(
            ...
            status=st,
            batch_month=batch_of(table),
            min_failed_rows=min_rows,
            warn_pct=warn,
        )
    )
...
def run_checks(df, table, checks):
    ...
    for i, c in enumerate(checks):
        aggs.append(F.sum(F.expr(f"CASE WHEN ({c.denom}) AND ({c.fail}) THEN 1 ELSE 0 END")).alias(f"f{i}"))
        aggs.append(F.sum(F.expr(f"CASE WHEN ({c.denom}) THEN 1 ELSE 0 END")).alias(f"t{i}"))
    row = df.agg(*aggs).collect()[0]
    for i, c in enumerate(checks):
        record(
            table, c.category, c.name, row[f"f{i}"] or 0, row[f"t{i}"] or 0, c.threshold, c.warn, c.min_rows, c.blocking
        )
...
#                reference it.
IGNORED_COLUMNS = {"green_taxi": {"ehail_fee"}, "weather": set(), "taxi_zones": set()}
...
def check_schema(df, table, expected, blocking=True):
    ...
    record(table, "schema", "expected_columns_present", len(missing), len(expected), STRICT, 0.0, 0, blocking)
    ...
    record(table, "schema", "no_unexpected_columns", len(extra), max(len(actual), 1), ADVISORY, 0.0, 0, False)
    if missing:
...
DURATION = "timestampdiff(SECOND, CAST(lpep_pickup_datetime AS TIMESTAMP), " "CAST(lpep_dropoff_datetime AS TIMESTAMP))"
...
taxi_checks = [
    ...
    Check(
        "passenger_count_not_null_excl_myle",
        "completeness",
        "passenger_count IS NULL",
        TOL,
        denom="VendorID <> 6 OR VendorID IS NULL",
    ),
    # -- validity ----------------------------------------------------------
    Check(
        "pickup_month_matches_source_file",
        "validity",
        f"date_format(CAST(lpep_pickup_datetime AS TIMESTAMP), 'yyyy-MM') <> '{BATCH_MONTH}'",
        TOL,
    ),
    ...
    Check(
        ...
        "consistency",
        "CAST(lpep_dropoff_datetime AS TIMESTAMP) < CAST(lpep_pickup_datetime AS TIMESTAMP)",
        TOL,
    ),
    Check(
        "duration_not_zero",
    ...
    Check("myle_dispatch_fields_stay_null", "consistency", "VendorID = 6 AND passenger_count IS NOT NULL", STRICT),
    # The six dispatch fields are null as a SET, never individually.
    Check(
        "dispatch_fields_null_as_a_set",
        "consistency",
        """(CASE WHEN passenger_count      IS NULL THEN 1 ELSE 0 END
            ...
            + CASE WHEN congestion_surcharge IS NULL THEN 1 ELSE 0 END) NOT IN (0, 6)""",
        STRICT,
    ),
    # -- business ----------------------------------------------------------
    ...
    Check(
        "total_equals_sum_of_charges_v2",
        "business",
    ...
    Check(
        "total_equals_sum_of_charges_v1",
        "business",
        """ABS(total_amount - (COALESCE(fare_amount,0) + COALESCE(extra,0)
    ...
    Check("myle_fare_stays_placeholder", "business", "fare_amount > 10", STRICT, denom="VendorID = 6"),
    ...
    Check(
        "fare_plausible_for_duration", "business", f"fare_amount > 100 AND trip_distance = 0 AND {DURATION} < 60", TOL
    ),
    Check("no_tip_recorded_on_cash", "business", "payment_type = 2 AND tip_amount > 0", TOL),
    Check("fare_implies_some_distance", "business", "fare_amount > 0 AND trip_distance = 0", TOL),
    Check("implied_speed_under_100mph", "business", f"try_divide(trip_distance, {DURATION} / 3600.0) > 100", TOL),
]
...
weather_checks = [
    ...
    Check(
        "gusts_at_least_wind_speed",
        "consistency",
        "try_cast(wind_gusts_10m AS DOUBLE) < try_cast(wind_speed_10m AS DOUBLE)",
        TOL,
    ),
    Check("temperature_plausible", "validity", "try_cast(temperature_2m AS DOUBLE) NOT BETWEEN -30 AND 50", TOL),
    Check(
        "precip_probability_0_to_100",
        "validity",
        "try_cast(precipitation_probability AS DOUBLE) NOT BETWEEN 0 AND 100",
        TOL,
    ),
    Check("cloud_cover_0_to_100", "validity", "try_cast(cloud_cover AS DOUBLE) NOT BETWEEN 0 AND 100", TOL),
...
zone_checks = [
    Check("location_id_not_null", "completeness", "LocationID IS NULL", STRICT, min_rows=0, blocking=True),
    ...
    Check(
        ...
        """LocationID IS NOT NULL AND (try_cast(LocationID AS INT) IS NULL
             OR try_cast(LocationID AS INT) NOT BETWEEN 1 AND 265)""",
        TOL,
    ),
    Check(
        "borough_in_domain",
    ...
    Check(
        "service_zone_in_domain",
        "validity",
        "service_zone NOT IN ('Boro Zone','Yellow Zone','Airports','EWR','N/A')",
        TOL,
    ),
]
...
if source_is_present("green_taxi", TAXI_DIR, TAXI_FILE):
    ...
    if n_taxi:
        check_schema(taxi, "green_taxi", TAXI_COLUMNS, blocking="green_taxi" in REQUIRED_TABLES)
        ...
        for c in taxi_checks:
            (runnable if all(col in have for col in columns_used(c, TAXI_COLUMNS)) else skipped).append(c)
        ...
        record("green_taxi", "uniqueness", "no_exact_duplicate_rows", dupes, total, TOL, 0.0, MIN_ROWS)
        print(f"  {n_taxi:,} rows")
...
if source_is_present("weather", WEATHER_DIR, WEATHER_FILE):
    ...
    weather_header = spark.read.option("header", "true").csv(f"{WEATHER_DIR}/{WEATHER_FILE}").limit(0).columns

    schema = StructType([StructField(c, StringType(), True) for c in WEATHER_COLUMNS])
    weather_raw = spark.read.option("header", "true").schema(schema).csv(f"{WEATHER_DIR}/{WEATHER_FILE}")
    # '' and '   ' become NULL once, here, rather than in every check.
    weather = weather_raw.select(
        *[F.when(F.trim(F.col(c)) == "", None).otherwise(F.trim(F.col(c))).alias(c) for c in weather_raw.columns]
    )
    n_weather = not_empty("weather", weather)
    if n_weather:
        check_schema(
            spark.createDataFrame([], StructType([StructField(c, StringType(), True) for c in weather_header])),
            "weather",
            WEATHER_COLUMNS,
        ...
        record("weather", "uniqueness", "one_row_per_hour", dupes, total, ADVISORY, 0.0, 0)
...
record(
    ...
    "schema",
    "expected_file_present",
    0 if zone_files else 1,
    1,
    STRICT,
    0.0,
    0,
    "taxi_zones" in REQUIRED_TABLES,
    is_empty_check=True,
)
if zone_files:
    zones = spark.read.option("header", "true").option("inferSchema", "false").csv(f"{ZONES_DIR}/")
    n_zones = not_empty("taxi_zones", zones)
    if n_zones:
        check_schema(zones, "taxi_zones", ZONE_COLUMNS, blocking="taxi_zones" in REQUIRED_TABLES)
        ...
        record("taxi_zones", "uniqueness", "location_id_unique", dupes, total, STRICT, 0.0, 0, blocking=True)
        # 265 rows and the three airport zones (1, 132, 138) are facts about
        # this reference file, so they are scalar checks, not rates.
        record("taxi_zones", "business", "lookup_has_265_zones", 0 if n_zones == 265 else 1, 1, STRICT, 0.0, 0)
        airports = (
            zones.where(F.expr("try_cast(LocationID AS INT) IN (1,132,138)")).select("LocationID").distinct().count()
        )
        record("taxi_zones", "business", "airport_zones_present", 0 if airports == 3 else 1, 1, STRICT, 0.0, 0)
...
skipped_sources = sorted(
    {r["table_name"] for r in RESULTS if r["check_name"] == "expected_file_present" and r["status"] == "FAIL"}
)
if skipped_sources:
...
for _tbl in ("green_taxi", "weather", "taxi_zones"):
    ...
    RESULTS.append(
        dict(
            ...
            table_name=_tbl,
            check_category="gate",
            check_name="source_cleared_for_bronze",
            failed_rows=len(_blocked),
            total_rows=1,
            failed_pct=100.0 if _blocked else 0.0,
            threshold_pct=0.0,
            status="FAIL" if _blocked else "PASS",
            batch_month=batch_of(_tbl),
            min_failed_rows=0,
            warn_pct=0.0,
        )
    )
...
runlog_absent = sorted(runlog_needed - {f.name for f in spark.table(RUNLOG_TABLE).schema.fields})
if runlog_absent:
    raise RuntimeError(f"{RUNLOG_TABLE} is missing {runlog_absent}. Run the DQ setup notebook first.")

target_cols = [f.name for f in spark.table(RESULTS_TABLE).schema.fields]
results_df = spark.createDataFrame([[r[f.name] for f in RESULTS_SCHEMA.fields] for r in RESULTS], RESULTS_SCHEMA)
...
out = results_df.select(
    *[
        F.col(c).cast(_types[c]) if c in results_df.columns else F.lit(None).cast(_types[c]).alias(c)
...
counts = {s: sum(1 for r in _checks if r["status"] == s) for s in ("PASS", "WARN", "FAIL", "SKIP")}
overall = "FAIL" if BLOCKING_FAILURES else "WARN" if counts["WARN"] or counts["FAIL"] else "PASS"
...
spark.createDataFrame(
    ...
    *[
        F.col(c).cast(dict(spark.table(RUNLOG_TABLE).dtypes)[c])
...
for name in BLOCKING_FAILURES:
    ...
    print(
        f"  [{tag}] {name}: {r['failed_rows']:,} of {r['total_rows']:,} "
...
try:
    for _t in ("green_taxi", "weather", "taxi_zones"):
        dbutils.jobs.taskValues.set(key=f"cleared_{_t}", value="PASS" if _t not in stopped else "FAIL")
    ...
    print(
        ...
        {f"cleared_{t}": ("PASS" if t not in stopped else "FAIL") for t in ("green_taxi", "weather", "taxi_zones")},
        "| batch_month:",
        BATCH_MONTH,
    )
