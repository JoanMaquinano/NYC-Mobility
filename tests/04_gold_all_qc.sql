...
SET VAR v_blocking_failures = (
    ...
    WHERE  run_id = v_run_id AND layer = 'gold' AND status = 'FAIL'
      AND  check_name IN (SELECT check_name FROM vw_gold_blocking_checks)
);
...
-- The blocking list checked against reality
WITH blocking AS (SELECT check_name FROM vw_gold_blocking_checks)
SELECT b.check_name AS blocking_name_never_produced
FROM       blocking b
LEFT  JOIN (SELECT DISTINCT check_name
            FROM   nyc_quality.dq_results
            WHERE  run_id = v_run_id AND layer = 'gold') r
...
SELECT v_blocking_failures AS blocking_failures,
       ...
       CASE WHEN v_blocking_failures > 0 THEN 'will stop: a key or a grain is broken'
            WHEN v_total_failures >= 5   THEN 'will stop: 5 or more failures'
            ELSE 'will continue' END     AS verdict;

SELECT CASE
    WHEN v_blocking_failures > 0
      THEN raise_error(CONCAT('Gold DQ gate FAILED (group): ',
                              CAST(v_blocking_failures AS STRING),
                              ' BLOCKING check(s) failed (of ',
                              CAST(v_total_failures AS STRING),
                              ' total): ', v_blocking_names, '. ',
                              'A dimension key is duplicated or a foreign key ',
                              'resolves to nothing -- every aggregate over this star is ',
                              'suspect until it is fixed. Run ', v_run_id))
    WHEN v_total_failures >= 5
      THEN raise_error(CONCAT('Gold DQ gate FAILED (group): ',
                              CAST(v_total_failures AS STRING),
                              ' checks over threshold, none individually blocking. Run ', v_run_id))
    ELSE CONCAT('Gold DQ gate PASSED (',
                CAST(v_total_failures AS STRING), ' non-blocking failure(s))')
END AS gate;

-- 7. Afterwards
--
-- All three layers side by side. This is the pipeline's whole quality
-- position in three rows, and it is the table for the write-up.

SELECT l.layer, l.overall_status, l.checks_run, l.checks_passed,
       l.checks_warned, l.checks_failed, l.run_ts
FROM       nyc_quality.dq_run_log l
INNER JOIN (SELECT layer, MAX(run_ts) AS mx
            FROM   nyc_quality.dq_run_log
            GROUP  BY layer) m
       ON  l.layer = m.layer AND l.run_ts = m.mx
ORDER  BY CASE l.layer WHEN 'bronze' THEN 1 WHEN 'silver' THEN 2 ELSE 3 END;
