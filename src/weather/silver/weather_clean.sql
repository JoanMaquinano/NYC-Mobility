...
USING (
  SELECT
    ...
    -- dq_status is a SUMMARY of the array and nothing else. Silver QC has
    -- three checks asserting this CASE holds on the rows in the table; they
    -- are impossible to fail unless this line is wrong, which is the point.
    CASE
  ...
  FROM (
    SELECT
      *,
      ARRAY_COMPACT(ARRAY(
        ...
        -- ---------------------------------------------------------------
        -- FAIL -- one condition. weather_hour is the join key for Gold.
        -- ---------------------------------------------------------------
        CASE WHEN weather_hour IS NULL
             THEN 'FAIL: date: hour could not be parsed to a timestamp' END,

        -- ---------------------------------------------------------------
        -- WARN -- present in the source and did not convert
        --
        -- NULLIF(trim(raw), '') rather than `raw IS NOT NULL`. An empty CSV
        -- cell arrives as '' which is NOT NULL, so the plain test reports the
        -- source's blanks as the pipeline's parse failures -- exactly the
        -- conflation the raw_ columns exist to prevent.
        -- ---------------------------------------------------------------
        CASE WHEN NULLIF(trim(raw_temperature_2m), '') IS NOT NULL
               AND temperature_2m IS NULL
             THEN 'WARN: temperature_2m: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_apparent_temperature), '') IS NOT NULL
               AND apparent_temperature IS NULL
             THEN 'WARN: apparent_temperature: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_precipitation_probability), '') IS NOT NULL
               AND precipitation_probability IS NULL
             THEN 'WARN: precipitation_probability: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_rain), '') IS NOT NULL AND rain IS NULL
             THEN 'WARN: rain: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_cloud_cover), '') IS NOT NULL AND cloud_cover IS NULL
             THEN 'WARN: cloud_cover: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_visibility), '') IS NOT NULL AND visibility IS NULL
             THEN 'WARN: visibility: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_wind_speed_10m), '') IS NOT NULL AND wind_speed_10m IS NULL
             THEN 'WARN: wind_speed_10m: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_wind_gusts_10m), '') IS NOT NULL AND wind_gusts_10m IS NULL
             THEN 'WARN: wind_gusts_10m: value did not parse' END,
        CASE WHEN NULLIF(trim(raw_weather_code), '') IS NOT NULL AND weather_code IS NULL
             THEN 'WARN: weather_code: value did not parse' END,

        -- ---------------------------------------------------------------
        -- WARN -- converted, but outside a plausible range
        -- ---------------------------------------------------------------
        ...
        CASE WHEN cloud_cover IS NOT NULL AND cloud_cover NOT BETWEEN 0.0 AND 100.0
        ...
        -- comes from -- Bronze sees 3 to 7 an hour-set. A column swap would
        -- show up as tens of percent instead.
        ...
        -- A code that parsed but is not in the WMO 4677 set the CASE knows
        -- about. The description silently becomes 'Unknown', so without this
        -- the only evidence is a string nobody queries.
        ...
        -- The month column disagreeing with the timestamp. Accepts YYYY-MM,
        -- M or MM, because the source convention is undocumented.
        CASE WHEN weather_hour IS NOT NULL AND NULLIF(trim(`month`), '') IS NOT NULL
               AND trim(`month`) <> date_format(weather_hour, 'yyyy-MM')
               AND COALESCE(try_cast(trim(`month`) AS INT), -1) <> month(weather_hour)
             THEN 'WARN: month: does not agree with the parsed hour' END,

        -- The loader writing the parameter name rather than its value,
        -- recorded per row rather than only in the run log.
    ...
    FROM (
      SELECT
        ...
        -- NULL, so a CSV that writes a code as 3.0 would report 100 percent
        ...
        -- Raw values kept alongside the typed ones, so a parse failure can be
        -- told apart from a value the source never sent. Without these, NULL
        -- means both things and the WARN rules cannot distinguish them.
    ...
    -- Deduplicate on the PARSED hour, falling back to the raw text when it
    -- did not parse -- otherwise every unparseable row shares a NULL key and
...
-- Keyed on the parsed hour, with `date` secondary so that unparseable rows,
-- whose hour is NULL, do not all match each other -- <=> treats NULL as equal
-- to NULL, which would collapse them on the hour alone.
...
-- ## Views — what Gold reads
--
-- Same pattern as green_taxi_clean. Nothing is deleted; the quarantine view
-- is a filter, so every excluded hour keeps its reasons.
CREATE OR REPLACE VIEW nyc_mobility.nyc_silver.vw_weather_valid
...
CREATE OR REPLACE VIEW nyc_mobility.nyc_silver.vw_weather_quarantined
...
-- Rows in neither view. Expect zero. If this returns rows, the CASE above did
-- not run on them -- a code fault, not a data fault, which is why
-- dq_status_populated blocks the pipeline.
CREATE OR REPLACE VIEW nyc_mobility.nyc_silver.vw_weather_unclassified
COMMENT 'Weather hours with a null or unrecognised dq_status. Always empty when the pipeline is correct.'
AS
SELECT *
FROM   nyc_mobility.nyc_silver.weather_clean
